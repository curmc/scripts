...
ROS_COMMAND="source /opt/ros/melodic/setup.zsh"
if grep -Fxq "$ROS_COMMAND" "$HOME/.zshrc"
then
	echo "All set up in rc"
else
	echo $ROS_COMMAND >> "$HOME/.zshrc"
	echo "Updated zshrc, please start a new shell and run again"
        exit
fi
